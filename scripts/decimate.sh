...
DATA_SOURCE=/home/jaehong/dataset/MM_release
CAM_POSE_SOURCE=${DATA_SOURCE}/camera_pose
OUTPUT_SOURCE=../output


#DECIM_RATIOS=(0.5 0.6 0.7 0.9 0.95 0.99)
DECIM_RATIOS=(0.6 0.7 0.95 0.99)
...
run_pipeline() {
    ...
    for decim in "${DECIM_RATIOS[@]}"; do
        echo "[RUN ] ${DATA_NAME} decim=${decim}"
        python reconstruct.py --rgbd_path ${DATA_DIR} \
            --frame_count ${FRAME_COUNT} --output_dir ${OUTPUT_DIR} \
            --reconstruct_geometry textured_mesh \
            --intrinsic_path ${INTRINSIC_PATH} --extrinsic_path ${EXTRINSIC_PATH} --decimation_ratio ${decim}
    done

    # python reconstruct.py --rgbd_path ${DATA_DIR} \
    #     --frame_count ${FRAME_COUNT} --output_dir ${OUTPUT_DIR} \
    #     --reconstruct_geometry textured_mesh \
    #     --intrinsic_path ${INTRINSIC_PATH} --extrinsic_path ${EXTRINSIC_PATH} --decimation_ratio 0.6

    # python reconstruct.py --rgbd_path ${DATA_DIR} \
    #     --frame_count ${FRAME_COUNT} --output_dir ${OUTPUT_DIR} \
    #     --reconstruct_geometry textured_mesh \
    #     --intrinsic_path ${INTRINSIC_PATH} --extrinsic_path ${EXTRINSIC_PATH} --decimation_ratio 0.7

    # python reconstruct.py --rgbd_path ${DATA_DIR} \
    #     --frame_count ${FRAME_COUNT} --output_dir ${OUTPUT_DIR} \
    #     --reconstruct_geometry textured_mesh \
    #     --intrinsic_path ${INTRINSIC_PATH} --extrinsic_path ${EXTRINSIC_PATH} --decimation_ratio 0.9
...
SCENE_DATASETS["couch"]="couch_scene0_100" #couch_scene1 couch_scene2 couch_scene3"
