...
{
  std::cout << "KinectCapture::get_factory_calibration() " << this->serial_num << std::endl;
...
void KinectCapture::capture_frame()
{
  ...
  // std::cout << "KinectCapture::capture_frame() " << this->serial_num << std::endl;
  ...
  if (!this->enable_depth_)
  {
    ...
    // Undistort (uncomment when using offline calibration - not factory calib.)
    cv::remap(
        this->cv_color_img,
    ...
    std::cout << "writing rectified color image" << std::endl;
  }
  else
  {
    this->colorImage = this->capture_.get_color_image();
    //this->colorImage = this->capture_.get_ir_image();
    this->depthImage = this->capture_.get_depth_image();

    // keep taking captures until both color and depth images are valid
    ...
    // cv::imwrite(this->serial_num + "_color_original.png", this->cv_color_img);
    // cv::imwrite(this->serial_num + "_depth_original.png", this->cv_depth_img);
    // cv::imwrite(this->serial_num + "_" + to_string(this->count) + "_color.png", this->cv_color_img_rectified);
    // cv::imwrite(this->serial_num + "_" + to_string(this->count) + "_depth.png", this->cv_depth_img_rectified);
    }
}
